...
export function useMessages(chatId: string) {
  ...
  // Send private tip (Encrypt FHE — amount hidden on-chain)
  const sendPrivateTipMutation = useMutation({
    mutationFn: async (amount: number) => {
      const response = await api.sendPrivateTip(chatId, amount);
      if (response.error) throw new Error(response.error);
      return response.data;
    },
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({ queryKey: ['chats'] });
      queryClient.invalidateQueries({ queryKey: ['wallet'] });
      toast.success('Private tip sent!');
    },
    onError: (error: Error) => {
      toast.error(error.message);
    },
  ...
  return {
    ...
    sendPrivateTip: sendPrivateTipMutation.mutate,
    isSendingPrivateTip: sendPrivateTipMutation.isPending,
